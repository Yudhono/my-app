...
type Props =
  ...
  | { qrOnly?: false; invoiceId: string; invoice: InvoiceData; paymentUrl?: never };
...
export default function InvoiceActions(props: Props) {
  ...
  const whatsappMessage = invoice.mayarPaymentUrl
    ? [
        `Halo ${invoice.clientName} 👋`,
        ``,
        `Berikut invoice untuk proyek *${invoice.projectDescription}*:`,
        ``,
        `📋 Invoice #${invoice.id}`,
    ...
    <div className="mb-4 flex flex-wrap gap-2 print:hidden">
      {whatsappUrl && (
        <a
          ...
          className="rounded-xl bg-green-500 px-4 py-2 text-sm font-semibold text-white hover:bg-green-600 transition-colors flex items-center gap-2"
        >
